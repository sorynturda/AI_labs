...
def restrict_move(dir):
    move = direction_queue[-1]
    if move == 'right':
        dir.remove('left')
    if move == 'down':
        dir.remove('up')
...
def on_key_down(key):
    ...
    dirCopy = dir.copy()
    dirCopy = restrict_move(dir)
    if key == keys.RIGHT or key == keys.LEFT or key == keys.UP or key == keys.DOWN:
        direction_queue.append(random.choice(dirCopy))
